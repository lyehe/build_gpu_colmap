...
### Key Components
- **COLMAP (latest)** - Structure-from-Motion and Multi-View Stereo (general use)
- **COLMAP for pycolmap** (colmap-for-pycolmap) - COLMAP configured specifically for Python wheel building
- **COLMAP 3.11** (colmap-for-glomap) - Pinned version specifically for GLOMAP compatibility
...
1. **Latest COLMAP** (`third_party/colmap`) → `build/install/colmap/`
   - For general use
   - Tracks latest COLMAP main branch
   - Full features enabled

2. **COLMAP for pycolmap** (`third_party/colmap-for-pycolmap`) → `build/install/colmap-for-pycolmap/`
...
3. **COLMAP 3.11** (`third_party/colmap-for-glomap`) → `build/install/colmap-for-glomap/`
   - Pinned to commit 78f1eefa (COLMAP 3.11)
   - Specifically for GLOMAP compatibility
...
Ceres Solver (base dependency)
    ├── COLMAP (latest) ─────────────→ build/install/colmap/
    ├── COLMAP for pycolmap (optional) → build/install/colmap-for-pycolmap/
